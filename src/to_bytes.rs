use plist::Error;
#[cfg(any(not(feature = "serde"), doc))]
use plist::{Data, Date, Dictionary, Integer, Uid, Value};

/// The formats a plist can be serialized to.
pub enum PlistFormat {
    XML,
    Binary,
}

/// If default features are enabled, this trait will automatically be implemented on any types that implement [`serde::Serialize`]. This allows you to use ToBytes on custom structs.
///
/// Otherwise, it will be implemented on all of plist's types ([`Data`], [`Date`], [`Dictionary`], [`Integer`], [`Uid`] (this cannot be serialized into XML), [`Value`]).
///
/// See [`ToBytes::to_bytes()`] for the method this trait provides and usage examples.
pub trait ToBytes {
    /// Serializes `self` to a plist with the specified format.
    ///
    /// # Examples
    ///
    /// ```rs
    /// use plist_ext::{Dictionary, DictionaryExt, PlistFormat, ToBytes};
    ///
    /// let mut dict = Dictionary::new();
    /// dict.add("key", "value");
    /// std::fs::write("./dict_xml", dict.to_bytes(PlistFormat::XML)).unwrap();
    /// std::fs::write("./dict_binary", dict.to_bytes(PlistFormat::Binary)).unwrap();
    /// ```
    fn to_bytes(&self, format: PlistFormat) -> Result<Vec<u8>, Error>;
}

macro_rules! to_bytes_fn {
    () => {
        fn to_bytes(&self, format: PlistFormat) -> Result<Vec<u8>, Error> {
            let mut bytes = vec![];
            match format {
                PlistFormat::XML => plist::to_writer_xml(&mut bytes, &self)?,
                PlistFormat::Binary => plist::to_writer_binary(&mut bytes, &self)?,
            };
            Ok(bytes)
        }
    };
}

macro_rules! impl_to_bytes {
    ($type:tt) => {
        #[cfg(not(feature = "serde"))]
        impl ToBytes for $type {
            to_bytes_fn!();
        }
    };
}

impl_to_bytes!(Data);
impl_to_bytes!(Date);
impl_to_bytes!(Dictionary);
impl_to_bytes!(Integer);
impl_to_bytes!(Value);
impl_to_bytes!(Uid);

#[cfg(feature = "serde")]
impl<T> ToBytes for T
where
    T: serde::Serialize,
{
    to_bytes_fn!();
}

#[cfg(test)]
mod tests {
    use plist::{Data, Date, Dictionary, Integer, Uid, Value};

    use crate::{DictionaryExt, PlistFormat, ToBytes};

    macro_rules! test_impl {
        ($type:expr, $name:ident, $output_xml:literal, $output_binary:literal) => {
            #[test]
            fn $name() {
                let plist = $type;

                // to help with adding new types (the assert failure output can be passed to convert_bytes.py)
                assert_eq!(
                    format!("{:x?}", plist.to_bytes(PlistFormat::XML).unwrap()),
                    format!("{:x?}", $output_xml),
                );
                assert_eq!(
                    format!("{:x?}", plist.to_bytes(PlistFormat::Binary).unwrap()),
                    format!("{:x?}", $output_binary),
                );

                assert_eq!(plist.to_bytes(PlistFormat::XML).unwrap(), $output_xml);
                assert_eq!(plist.to_bytes(PlistFormat::Binary).unwrap(), $output_binary);
            }
        };
    }

    test_impl!(
        Data::new(vec![0u8, 0u8, 0u8, 0u8]),
        data,
        b"",
        b"\x62\x70\x6c\x69\x73\x74\x30\x30\x44\x00\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0d"
    );

    test_impl!(
        Date::from_xml_format("1981-05-16T11:32:06Z").unwrap(),
        date,
        b"",
        b"\x62\x70\x6c\x69\x73\x74\x30\x30\x33\xc1\xc2\x76\x00\x65\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x11"
    );

    test_impl!(
        Dictionary::with([("integer", 1.into()), ("string", "value".into())]),
        dictionary,
        b"",
        b""
    );

    test_impl!(
        Integer::from(123),
        integer,
        b"",
        b"\x62\x70\x6c\x69\x73\x74\x30\x30\x10\x7b\x08\x00\x00\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0a"
    );

    #[test]
    fn uid() {
        const OUTPUT_BINARY: &[u8] = b"\x62\x70\x6c\x69\x73\x74\x30\x30\x80\x7b\x08\x00\x00\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0a";

        let plist = Uid::new(123);

        // to help with adding new types (the assert failure output can be passed to convert_bytes.py)
        assert_eq!(
            format!("{:x?}", plist.to_bytes(PlistFormat::Binary).unwrap()),
            format!("{:x?}", OUTPUT_BINARY),
        );

        assert_eq!(plist.to_bytes(PlistFormat::Binary).unwrap(), OUTPUT_BINARY);
    }

    test_impl!(
        Value::from(123),
        value,
        b"",
        b"\x62\x70\x6c\x69\x73\x74\x30\x30\x10\x7b\x08\x00\x00\x00\x00\x00\x00\x01\x01\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x0a"
    );

    #[cfg(feature = "serde")]
    #[derive(serde::Serialize)]
    #[serde(rename_all = "PascalCase")]
    struct TestStruct {
        property: String,
        another_property: i64,
        a_date: Date,
    }

    #[cfg(feature = "serde")]
    test_impl!(
        TestStruct {
            property: "value".into(),
            another_property: 456.into(),
            a_date: std::time::SystemTime::UNIX_EPOCH.into(),
        },
        serde_serialize,
        b"",
        b""
    );
}
